...
using MyAssistant1.Responses.Cancel;
...
namespace MyAssistant1.Dialogs
{
	public class MainDialog : RouterDialog
	{
		...
		public MainDialog(
			...
			RateAnswerDialog rateAnswerDialog,
		...
		protected override async Task RouteAsync(DialogContext dc, CancellationToken cancellationToken = default(CancellationToken))
		{
			...
			else if (intent == DispatchLuis.Intent.q_Faq)
			{
				...
				else
				{
					...
					{
						Microsoft.Bot.Builder.AI.QnA.QueryResult highestRatedAnswerResult = answers[0];
						string answerText = highestRatedAnswerResult.Answer;

						await dc.Context.SendActivityAsync(answerText, speak: answerText);

						//Microsoft.Bot.Builder.AI.QnA.QueryResult highestRatedAnswerResult = new Microsoft.Bot.Builder.AI.QnA.QueryResult() { Answer = "I am an answer" };

						// Ask for feedback on non - chitchat items
						if (highestRatedAnswerResult.Metadata.All(a => a.Value != "chitchat"))
						{
							await dc.BeginDialogAsync(nameof(RateAnswerDialog), new { Question = dc.Context.Activity.Text });
						}
					}
					else
					{
